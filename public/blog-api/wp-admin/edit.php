...
if ($post_type === 'post') {
    ...
    get_current_screen()->add_help_tab(
        [
            ...
            'content' => '<p>' . __('Hovering over a row in the posts list will display action links that allow you to manage your post. You can perform the following actions:') . '</p>' .
                    '<ul>' .
    ...
    get_current_screen()->add_help_tab(
        [
            ...
            'content' => '<p>' . __('You can also edit or move multiple posts to the Trash at once. Select the posts you want to act on using the checkboxes, then select the action you want to take from the Bulk actions menu and click Apply.') . '</p>' .
                    '<p>' . sprintf(
    ...
    get_current_screen()->add_help_tab(
        [
            ...
            'content' => '<p>' . __('Managing pages is very similar to managing posts, and the screens can be customized in the same way.') . '</p>' .
...
$bulk_messages['wp_block'] = [
    ...
    'locked' => ($bulk_counts['locked'] === 1) ? __('1 pattern not updated, somebody is editing it.') :
                    /* translators: %s: Number of patterns. */
