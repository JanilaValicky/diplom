...
get_current_screen()->add_help_tab(
    [
        'id' => 'overview',
        'title' => __('Overview'),
        'content' => '<p>' . __('Add New takes you to the Add New Site screen. You can search for a site by Name, ID number, or IP address. Screen Options allows you to choose how many sites to display on one page.') . '</p>' .
...
if (isset($_GET['action'])) {
    ...
    switch ($_GET['action']) {
        ...
        case 'deleteblog':
            ...
            if ($id !== 0 && !is_main_site($id) && current_user_can('delete_site', $id)) {
        ...
        case 'allblogs':
            if (isset($_POST['action']) && isset($_POST['allblogs'])) {
                ...
                foreach ((array) $_POST['allblogs'] as $site_id) {
                    ...
                    if ($site_id !== 0 && !is_main_site($site_id)) {
                        switch ($doaction) {
                            case 'delete':
                                require_once ABSPATH . 'wp-admin/admin-header.php';
                                ?>
                            ...
                            case 'notspam':
                                ...
                                update_blog_status($site_id, 'spam', ($doaction === 'spam') ? '1' : '0');
                                break;
                        }
                    } else {
                        wp_die(__('Sorry, you are not allowed to change the current site.'));
