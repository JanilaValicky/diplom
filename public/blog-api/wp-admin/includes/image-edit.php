...
function wp_image_editor($post_id, $msg = false)
{
    ...
    if (isset($meta['width'], $meta['height'])) {
        ...
        exit(__('Image data does not exist. Please re-upload the image.'));
    }
...
function wp_save_image($post_id)
{
    ...
    $scale = !empty($_REQUEST['do']) && $_REQUEST['do'] === 'scale';
    ...
    if (defined('IMAGE_EDIT_OVERWRITE') && IMAGE_EDIT_OVERWRITE
        ...
        if ($edit_thumbnails_separately && $target === 'thumbnail') {
    ...
    if ($target === 'nothumb' || $target === 'all' || $target === 'full' || $scaled) {
        $tag = false;
        ...
        if ($success && ($target === 'nothumb' || $target === 'all')) {
            $sizes = get_intermediate_image_sizes();

            if ($edit_thumbnails_separately && $target === 'nothumb') {
    ...
    } elseif ($edit_thumbnails_separately && $target === 'thumbnail') {
        $sizes = ['thumbnail'];
        $success = true;
        $delete = true;
        $nocrop = true;
    ...
    if ($success) {
        ...
        if ($target === 'thumbnail' || $target === 'all' || $target === 'full') {
            ...
            if (!empty($_REQUEST['context']) && $_REQUEST['context'] === 'edit-attachment') {
