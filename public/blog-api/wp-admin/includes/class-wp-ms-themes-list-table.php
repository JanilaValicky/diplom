...
class WP_MS_Themes_List_Table extends WP_List_Table
{
    ...
    public function __construct($args = [])
    {
        ...
        if (!in_array($status, ['all', 'enabled', 'disabled', 'upgrade', 'search', 'broken', 'auto-update-enabled', 'auto-update-disabled'], true)) {
            $status = 'all';
        }

        $page = $this->get_pagenum();

        $this->is_site_themes = ($this->screen->id === 'site-themes-network') ? true : false;
    ...
    public function _search_callback($theme)
    {
        ...
        foreach (['Name', 'Description', 'Author', 'Author', 'AuthorURI'] as $field) {
            ...
            if (stripos($theme->display($field, false, true), $term) !== false) {
                return true;
            }
        }

        if (stripos($theme->get_stylesheet(), $term) !== false) {
            return true;
        }

        if (stripos($theme->get_template(), $term) !== false) {
            return true;
        }
