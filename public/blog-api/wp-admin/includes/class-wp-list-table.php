...
class WP_List_Table
{
    ...
    {
        exit('function WP_List_Table::ajax_user_can() must be overridden in a subclass.');
    }
    ...
    {
        exit('function WP_List_Table::prepare_items() must be overridden in a subclass.');
    }
...
<p class="search-box">
    ...
    protected function get_views_links($link_data = [])
    {
        ...
        foreach ($link_data as $view => $link) {
            ...
            $views_links[$view] = sprintf(
                '<a href="%s"%s>%s</a>',
                esc_url($link['url']),
                isset($link['current']) && $link['current'] === true ? ' class="current" aria-current="page"' : '',
                $link['label']
            );
        }
    ...
    {
        exit('function WP_List_Table::get_columns() must be overridden in a subclass.');
    }
        ...
        foreach ($columns as $column_key => $column_display_name) {
            ...
            if (isset($sortable[$column_key])) {
                ...
                if ($current_orderby === $orderby) {
                    ...
                    $order_text = $order === 'asc' ? $asc_text : $desc_text;
                }
        ...
        foreach (array_keys($columns) as $column_key) {
            ...
            if (isset($sortable[$column_key])) {
                ...
                if ($current_orderby === $orderby) {
                    ...
                    $order_text = $current_order === 'asc' ? $asc_text : $desc_text;
