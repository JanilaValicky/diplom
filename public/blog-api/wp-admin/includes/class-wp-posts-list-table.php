...
class WP_Posts_List_Table extends WP_List_Table
{
    ...
    public function __construct($args = [])
    {
        ...
        if ($post_type === 'post' && $sticky_posts) {
    ...
    protected function is_base_request()
    {
        ...
        if (empty($vars)) {
            return true;
        } elseif (count($vars) === 1 && !empty($vars['post_type'])) {
            return $this->screen->post_type === $vars['post_type'];
        }

        return count($vars) === 1 && !empty($vars['mode']);
    }
    ...
    public function column_title($post)
    {
        ...
        if ($this->hierarchical_display) {
            if ($this->current_level === 0 && (int) $post->post_parent > 0) {
    ...
    protected function handle_row_actions($item, $column_name, $primary)
    {
        ...
        if ($can_edit_post && $post->post_status !== 'trash') {
            ...
            if ($quick_edit_enabled && $post->post_type !== 'wp_block') {
    ...
    public function inline_edit()
    {
        ...
        $m = (isset($mode) && $mode === 'excerpt') ? 'excerpt' : 'list';
        ...
        while ($bulk < 2) {
			...
			<fieldset class="inline-edit-col-left">
				...
				<?php if (post_type_supports($screen->post_type, 'title')) { ?>
					...
					<?php } else { // $bulk?>
						...
						<?php } // is_post_type_viewable()?>

					<?php } // $bulk?>

				<?php } // post_type_supports( ... 'title' )?>
			...
			<?php if (count($hierarchical_taxonomies) && !$bulk) { ?>
				...
				<fieldset class="inline-edit-col-center inline-edit-categories">
					...
					<?php } // $hierarchical_taxonomies as $taxonomy?>

					</div>
				</fieldset>

			<?php } // count( $hierarchical_taxonomies ) && ! $bulk?>
            ...
            ?>
				...
				<?php } // post_type_supports( ... 'page-attributes' )?>

				<?php if (count(get_page_templates(null, $screen->post_type)) > 0) { ?>
				...
				<?php if (count($flat_taxonomies) && !$bulk) { ?>
					...
					<?php foreach ($flat_taxonomies as $taxonomy) { ?>
						...
						<?php } // current_user_can( 'assign_terms' )?>

					<?php } // $flat_taxonomies as $taxonomy?>

				<?php } // count( $flat_taxonomies ) && ! $bulk?>
				...
				<?php } // post_type_supports( ... comments or pings )?>
					...
					<div class="inline-edit-group wp-clearfix">
						...
						<?php if ($screen->post_type === 'post' && $can_publish && current_user_can($post_type_object->cap->edit_others_posts)) { ?>
						...
						<?php } // 'post' && $can_publish && current_user_can( 'edit_others_posts' )?>
