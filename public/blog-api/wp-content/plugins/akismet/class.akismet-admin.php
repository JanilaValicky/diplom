...
class Akismet_Admin
{
    ...
    public static function init_hooks()
    {
        ...
        if (isset($_GET['page']) && $_GET['page'] == 'akismet-stats-display') {
        ...
        add_action('admin_init', ['Akismet_Admin', 'admin_init']);
        add_action('admin_menu', ['Akismet_Admin', 'admin_menu'], 5); // Priority 5, so it's called before Jetpack's admin_menu.
        add_action('admin_notices', ['Akismet_Admin', 'display_notice']);
        add_action('admin_enqueue_scripts', ['Akismet_Admin', 'load_resources']);
        add_action('activity_box_end', ['Akismet_Admin', 'dashboard_stats']);
        add_action('rightnow_end', ['Akismet_Admin', 'rightnow_stats']);
        add_action('manage_comments_nav', ['Akismet_Admin', 'check_for_spam_button']);
        add_action('admin_action_akismet_recheck_queue', ['Akismet_Admin', 'recheck_queue']);
        add_action('wp_ajax_akismet_recheck_queue', ['Akismet_Admin', 'recheck_queue']);
        add_action('wp_ajax_comment_author_deurl', ['Akismet_Admin', 'remove_comment_author_url']);
        add_action('wp_ajax_comment_author_reurl', ['Akismet_Admin', 'add_comment_author_url']);
        add_action('jetpack_auto_activate_akismet', ['Akismet_Admin', 'connect_jetpack_user']);

        add_filter('plugin_action_links', ['Akismet_Admin', 'plugin_action_links'], 10, 2);
        add_filter('comment_row_actions', ['Akismet_Admin', 'comment_row_action'], 10, 2);

        add_filter('plugin_action_links_' . plugin_basename(plugin_dir_path(__FILE__) . 'akismet.php'), ['Akismet_Admin', 'admin_plugin_settings_link']);

        add_filter('wxr_export_skip_commentmeta', ['Akismet_Admin', 'exclude_commentmeta_from_export'], 10, 3);

        add_filter('all_plugins', ['Akismet_Admin', 'modify_plugin_description']);

        // priority=1 because we need ours to run before core's comment anonymizer runs, and that's registered at priority=10
        add_filter('wp_privacy_personal_data_erasers', ['Akismet_Admin', 'register_personal_data_eraser'], 1);
    }
    ...
    public static function admin_init()
    {
        ...
        add_meta_box('akismet-status', __('Comment History', 'akismet'), ['Akismet_Admin', 'comment_status_meta_box'], 'comment', 'normal');
    ...
    public static function admin_menu()
    {
        if (class_exists('Jetpack')) {
            add_action('jetpack_admin_menu', ['Akismet_Admin', 'load_menu']);
        } else {
            self::load_menu();
        }
    }
    ...
    {
        if (class_exists('Jetpack')) {
            $hook = add_submenu_page('jetpack', __('Akismet Anti-spam', 'akismet'), __('Akismet Anti-spam', 'akismet'), 'manage_options', 'akismet-key-config', ['Akismet_Admin', 'display_page']);
        } else {
            $hook = add_options_page(__('Akismet Anti-spam', 'akismet'), __('Akismet Anti-spam', 'akismet'), 'manage_options', 'akismet-key-config', ['Akismet_Admin', 'display_page']);
        }

        if ($hook) {
            add_action("load-$hook", ['Akismet_Admin', 'admin_help']);
        }
    }
    ...
    public static function load_resources()
    {
        ...
        if (in_array($hook_suffix, apply_filters('akismet_admin_page_hook_suffixes', [
            'index.php', // dashboard
            'edit-comments.php',
            'comment.php',
            'post.php',
            ...
            wp_register_style('akismet', plugin_dir_url(__FILE__) . $akismet_css_path, [], filemtime(dirname(__FILE__) . '/' . $akismet_css_path));
            wp_enqueue_style('akismet');

            wp_register_style('akismet-font-inter', plugin_dir_url(__FILE__) . '_inc/fonts/inter.css', [], filemtime(dirname(__FILE__) . '/_inc/fonts/inter.css'));
            wp_enqueue_style('akismet-font-inter');

            $akismet_admin_css_path = is_rtl() ? '_inc/rtl/akismet-admin-rtl.css' : '_inc/akismet-admin.css';
            wp_register_style('akismet-admin', plugin_dir_url(__FILE__) . $akismet_admin_css_path, [], filemtime(dirname(__FILE__) . '/' . $akismet_admin_css_path));
            wp_enqueue_style('akismet-admin');

            wp_register_script('akismet.js', plugin_dir_url(__FILE__) . '_inc/akismet.js', ['jquery'], AKISMET_VERSION);
            wp_enqueue_script('akismet.js');

            wp_register_script('akismet-admin.js', plugin_dir_url(__FILE__) . '_inc/akismet-admin.js', [], filemtime(dirname(__FILE__) . '/_inc/akismet-admin.js'));
            ...
            if (!Akismet::get_api_key() || (isset($_GET['view']) && $_GET['view'] == 'start')) {
                ...
                $current_screen->add_help_tab(
                    [
                        ...
                        'content' => '<p><strong>' . esc_html__('Akismet Setup', 'akismet') . '</strong></p>' .
                ...
                $current_screen->add_help_tab(
                    [
                        'id' => 'overview',
                        'title' => __('Overview', 'akismet'),
                        'content' => '<p><strong>' . esc_html__('Akismet Stats', 'akismet') . '</strong></p>' .
                            '<p>' . esc_html__('Akismet filters out spam, so you can focus on more important things.', 'akismet') . '</p>' .
                ...
                $current_screen->add_help_tab(
                    [
                        'id' => 'overview',
                        'title' => __('Overview', 'akismet'),
                        'content' => '<p><strong>' . esc_html__('Akismet Configuration', 'akismet') . '</strong></p>' .
                            '<p>' . esc_html__('Akismet filters out spam, so you can focus on more important things.', 'akismet') . '</p>' .
    ...
    public static function enter_api_key()
    {
        ...
        foreach (['akismet_strictness', 'akismet_show_user_comments_approved'] as $option) {
    ...
    public static function save_key($api_key)
    {
        ...
        if ($key_status == 'valid') {
            ...
            if ($akismet_user) {
                if (in_array($akismet_user->status, ['active', 'active-dunning', 'no-sub'])) {
        ...
        } elseif (in_array($key_status, ['invalid', 'failed'])) {
    ...
    public static function dashboard_stats()
    {
        ...
        echo '<p>' . sprintf(_n(
            ...
            $count, 'akismet'), 'https://akismet.com/wordpress/', esc_url(add_query_arg(['page' => 'akismet-admin'], admin_url(isset($submenu['edit-comments.php']) ? 'edit-comments.php' : 'edit.php'))), number_format_i18n($count)) . '</p>';
    }
    ...
    public static function rightnow_stats()
    {
        if ($count = get_option('akismet_spam_count')) {
            $intro = sprintf(_n(
                ...
                $count, 'akismet'), 'https://akismet.com/wordpress/', number_format_i18n($count));
        } else {
        ...
        if ($queue_count = self::get_spam_count()) {
            $queue_text = sprintf(_n(
                ...
                $queue_count, 'akismet'), number_format_i18n($queue_count), esc_url($link));
        } else {
    ...
    public static function check_for_spam_button($comment_status)
    {
        ...
        if ($comment_status != 'all' && $comment_status != 'moderated') {
            return;
        }
        ...
        echo '<a
				...
				data-success-url="' . esc_attr(remove_query_arg(['akismet_recheck', 'akismet_recheck_error'], add_query_arg(['akismet_recheck_complete' => 1, 'recheck_count' => urlencode('__recheck_count__'), 'spam_count' => urlencode('__spam_count__')]))) . '"
				data-failure-url="' . esc_attr(remove_query_arg(['akismet_recheck', 'akismet_recheck_complete'], add_query_arg(['akismet_recheck_error' => 1]))) . '"
    ...
    public static function recheck_queue()
    {
        ...
        if (!(isset($_GET['recheckqueue']) || (isset($_REQUEST['action']) && $_REQUEST['action'] == 'akismet_recheck_queue'))) {
            return;
        }
    ...
    public static function remove_comment_author_url()
    {
        if (!empty($_POST['id']) && check_admin_referer('comment_author_url_nonce')) {
            ...
            if ($comment && current_user_can('edit_comment', $comment['comment_ID'])) {
                ...
                echo wp_update_comment($comment);
                exit();
            }
        }
    }
    ...
    public static function get_spam_count($type = false)
    {
        ...
        if (!$type) { // total
            ...
            return $count;
        } elseif ($type == 'comments' || $type == 'comment') { // comments
            $type = '';
        }
    ...
    public static function check_server_connectivity($cache_timeout = 86400)
    {
        ...
        if ($response && wp_remote_retrieve_body($response) == 'connected') {
            return true;
        }
    ...
    public static function get_page_url($page = 'config')
    {
        ...
        $args = ['page' => 'akismet-key-config'];

        if ($page == 'stats') {
            $args = ['page' => 'akismet-key-config', 'view' => 'stats'];
        } elseif ($page == 'delete_key') {
            $args = ['page' => 'akismet-key-config', 'view' => 'start', 'action' => 'delete-key', '_wpnonce' => wp_create_nonce(self::NONCE)];
        } elseif ($page === 'init') {
            $args = ['page' => 'akismet-key-config', 'view' => 'start'];
        }
    ...
    public static function get_akismet_user($api_key)
    {
        ...
        $subscription_verification = Akismet::http_post(Akismet::build_query(['key' => $api_key, 'blog' => get_option('home')]), 'get-subscription');

        if (!empty($subscription_verification[1])) {
            if ($subscription_verification[1] !== 'invalid') {
    ...
    public static function get_stats($api_key)
    {
        ...
        foreach (['6-months', 'all'] as $interval) {
            $response = Akismet::http_post(Akismet::build_query(['blog' => get_option('home'), 'key' => $api_key, 'from' => $interval]), 'get-stats');
    ...
    {
        $akismet_account = Akismet::http_post(Akismet::build_query(array_merge([
            'user_id' => $user_id,
            'api_key' => $api_key,
    ...
    public static function connect_jetpack_user()
    {
        ...
        if ($jetpack_user = self::get_jetpack_user()) {
            if (isset($jetpack_user['user_id']) && isset($jetpack_user['api_key'])) {
                $akismet_user = self::verify_wpcom_key($jetpack_user['api_key'], $jetpack_user['user_id'], ['action' => 'connect_jetpack_user']);

                if (is_object($akismet_user)) {
                    self::save_key($akismet_user->api_key);

                    return in_array($akismet_user->status, ['active', 'active-dunning', 'no-sub']);
                }
            }
        }
    ...
    public static function display_spam_check_warning()
    {
        ...
        if (wp_next_scheduled('akismet_schedule_cron_recheck') > time() && self::are_any_comments_waiting_to_be_checked()) {
            ...
            if (defined('DISABLE_WP_CRON') && DISABLE_WP_CRON && apply_filters('akismet_display_cron_disabled_notice', true)) {
                Akismet::view('notice', ['type' => 'spam-check-cron-disabled']);
            } else {
                ...
                Akismet::view('notice', ['type' => 'spam-check', 'link_text' => $link_text]);
            }
        }
    }
    ...
    {
        Akismet::view('notice', ['type' => 'plugin']);
    }
    ...
    public static function display_start_page()
    {
        ...
        if ($api_key = Akismet::get_api_key() && (empty(self::$notices['status']) || self::$notices['status'] != 'existing-key-invalid')) {
            self::display_configuration_page();
        ...
        if (isset($_GET['token']) && preg_match('/^(\d+)-[0-9a-f]{20}$/', $_GET['token'])) {
            $akismet_user = self::verify_wpcom_key('', '', ['token' => $_GET['token']]);
    ...
    public static function display_configuration_page()
    {
        ...
        if (empty(self::$notices)) {
            if (!empty($stat_totals['all']) && isset($stat_totals['all']->time_saved) && $akismet_user->status == 'active' && $akismet_user->account_type == 'free-api-key') {
                ...
                $notices[] = ['type' => 'active-notice', 'time_saved' => $time_saved];
            }
        }

        if (!isset(self::$notices['status']) && in_array($akismet_user->status, ['cancelled', 'suspended', 'missing', 'no-sub'])) {
            $notices[] = ['type' => $akismet_user->status];
        }
    ...
    public static function display_notice()
    {
        ...
        if (in_array($hook_suffix, ['jetpack_page_akismet-key-config', 'settings_page_akismet-key-config'])) {
            // This page manages the notices and puts them inline where they make sense.
            return;
        }

        if (in_array($hook_suffix, ['edit-comments.php']) && (int) get_option('akismet_alert_code') > 0) {
        ...
        } elseif (($hook_suffix === 'plugins.php' || $hook_suffix === 'edit-comments.php') && !Akismet::get_api_key()) {
    ...
    public static function display_status()
    {
        if (!self::get_server_connectivity()) {
            Akismet::view('notice', ['type' => 'servers-be-down']);
    ...
    private static function get_jetpack_user()
    {
        ...
        $xml = new Jetpack_IXR_ClientMulticall(['user_id' => get_current_user_id()]);
    ...
    {
        if (in_array($key, ['akismet_as_submitted', 'akismet_rechecking', 'akismet_delayed_moderation_email'])) {
            return true;
        }
    ...
    {
        if (in_array($state, ['display', 'hide'])) {
    ...
    public static function register_personal_data_eraser($erasers)
    {
        $erasers['akismet'] = [
            ...
            'callback' => ['Akismet_Admin', 'erase_personal_data'],
        ];
