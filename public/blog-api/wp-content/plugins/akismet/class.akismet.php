...
class Akismet
{
    ...
    private static $comment_as_submitted_allowed_keys = ['blog' => '', 'blog_charset' => '', 'blog_lang' => '', 'blog_ua' => '', 'comment_agent' => '', 'comment_author' => '', 'comment_author_IP' => '', 'comment_author_email' => '', 'comment_author_url' => '', 'comment_content' => '', 'comment_date_gmt' => '', 'comment_tags' => '', 'comment_type' => '', 'guid' => '', 'is_test' => '', 'permalink' => '', 'reporter' => '', 'site_domain' => '', 'submit_referer' => '', 'submit_uri' => '', 'user_ID' => '', 'user_agent' => '', 'user_id' => '', 'user_ip' => ''];
    ...
    private static function init_hooks()
    {
        ...
        add_filter('preprocess_comment', ['Akismet', 'auto_check_comment'], 1);
        add_filter('rest_pre_insert_comment', ['Akismet', 'rest_auto_check_comment'], 1);

        add_action('comment_form', ['Akismet', 'load_form_js']);
        add_action('do_shortcode_tag', ['Akismet', 'load_form_js_via_filter'], 10, 4);
        ...
        add_action('akismet_scheduled_delete', ['Akismet', 'delete_orphaned_commentmeta']);
        add_action('akismet_schedule_cron_recheck', ['Akismet', 'cron_recheck']);

        add_action('comment_form', ['Akismet',  'add_comment_nonce'], 1);
        add_action('comment_form', ['Akismet', 'output_custom_form_fields']);
        add_filter('script_loader_tag', ['Akismet', 'set_form_js_async'], 10, 3);
        ...
        add_action('xmlrpc_call', ['Akismet', 'pre_check_pingback']);

        // Jetpack compatibility
        add_filter('jetpack_options_whitelist', ['Akismet', 'add_to_jetpack_options_whitelist']);
        add_filter('jetpack_contact_form_html', ['Akismet', 'inject_custom_form_fields']);
        add_filter('jetpack_contact_form_akismet_values', ['Akismet', 'prepare_custom_form_values']);

        // Gravity Forms
        add_filter('gform_get_form_filter', ['Akismet', 'inject_custom_form_fields']);
        add_filter('gform_akismet_fields', ['Akismet', 'prepare_custom_form_values']);

        // Contact Form 7
        add_filter('wpcf7_form_elements', ['Akismet', 'append_custom_form_fields']);
        add_filter('wpcf7_akismet_parameters', ['Akismet', 'prepare_custom_form_values']);

        // Formidable Forms
        add_filter('frm_filter_final_form', ['Akismet', 'inject_custom_form_fields']);
        add_filter('frm_akismet_values', ['Akismet', 'prepare_custom_form_values']);
         ...
         */
        add_filter('fluentform_form_element_start', ['Akismet', 'output_custom_form_fields']);
        add_filter('fluentform_akismet_fields', ['Akismet', 'prepare_custom_form_values'], 10, 2);
        // Current Fluent Form hooks.
        add_filter('fluentform/form_element_start', ['Akismet', 'output_custom_form_fields']);
        add_filter('fluentform/akismet_fields', ['Akismet', 'prepare_custom_form_values'], 10, 2);

        add_action('update_option_wordpress_api_key', ['Akismet', 'updated_option'], 10, 2);
        add_action('add_option_wordpress_api_key', ['Akismet', 'added_option'], 10, 2);

        add_action('comment_form_after', ['Akismet',  'display_comment_form_privacy_notice']);
    }
    ...
    public static function get_access_token()
    {
        ...
        if (is_null($access_token)) {
            $response = self::http_post(self::build_query(['api_key' => self::get_api_key()]), 'token');
    ...
    {
        $response = self::http_post(Akismet::build_query(['key' => $key, 'blog' => get_option('home')]), 'deactivate');
    ...
    {
        if ($option_name === 'wordpress_api_key') {
    ...
    public static function auto_check_comment($commentdata, $context = 'default')
    {
        ...
        if ($response[1] != 'true' && $response[1] != 'false') {
            ...
            if (self::matches_last_comment($comment)) {
                ...
                if (self::$last_comment['akismet_result'] == 'true') {
                    ...
                    if ($comment->comment_approved == 'spam' || $comment->comment_approved == 'trash') {
                        ...
                        } elseif (function_exists('wp_blacklist_check') && wp_blacklist_check($comment->comment_author, $comment->comment_author_email, $comment->comment_author_url, $comment->comment_content, $comment->comment_author_IP, $comment->comment_agent)) {
                    ...
                    self::update_comment_history(
                        ...
                        '',
                        'check-error',
                        ['response' => substr(self::$last_comment['akismet_result'], 0, 50)]
                    );
                }
    ...
    public static function delete_orphaned_commentmeta()
    {
        ...
        while ($commentmeta_results = $wpdb->get_results($wpdb->prepare("SELECT m.meta_id, m.comment_id, m.meta_key FROM {$wpdb->commentmeta} as m LEFT JOIN {$wpdb->comments} as c USING(comment_id) WHERE c.comment_id IS NULL AND m.meta_id > %d ORDER BY m.meta_id LIMIT 1000", $last_meta_id))) {
            ...
            foreach ($commentmeta_results as $commentmeta) {
                if (substr($commentmeta->meta_key, 0, 8) == 'akismet_') {
    ...
    public static function get_user_comments_approved($user_id, $comment_author_email, $comment_author, $comment_author_url)
    {
         ...
         * @since 4.2.2
         */
        $excluded_comment_types = apply_filters('akismet_excluded_comment_types', []);
    ...
    public static function recheck_comment($id, $recheck_reason = 'recheck_queue')
    {
        ...
        if (is_wp_error($api_response)) {
            ...
            Akismet::update_comment_history(
                ...
                '',
                'recheck-error',
                ['response' => substr($api_response, 0, 50)]
            );
        }
    ...
    public static function transition_comment_status($new_status, $old_status, $comment)
    {
        ...
        if (
            ...
            (isset($_POST['status']) && in_array($_POST['status'], ['spam', 'unspam', 'approved', ]))
            ...
            || (isset($_GET['action']) && in_array($_GET['action'], ['spam', 'unspam', 'spamcomment', 'unspamcomment', ]))
            ...
            || (isset($_GET['for']) && ($_GET['for'] == 'jetpack') && (!defined('IS_WPCOM') || !IS_WPCOM))
    ...
    public static function http_post($request, $path, $ip = null)
    {
        ...
        $simplified_response = [$response['headers'], $response['body']];
    ...
    public static function get_akismet_form_fields()
    {
        ...
        if (current_filter() === 'wpcf7_form_elements') {
            $prefix = '_wpcf7_ak_';
        }
    ...
    {
        if (current_filter() === 'fluentform/form_element_start' && did_action('fluentform_form_element_start')) {
    ...
    {
        if (current_filter() === 'fluentform/akismet_fields' && did_filter('fluentform_akismet_fields')) {
        ...
        // Contact Form 7 uses _wpcf7 as a prefix to know which fields to exclude from comment_content.
        if (current_filter() === 'wpcf7_akismet_parameters') {
            $prefix = '_wpcf7_ak_';
        }
...
<?php
    ...
    public static function plugin_activation()
    {
        ...
        } elseif (!empty($_SERVER['SCRIPT_NAME']) && strpos($_SERVER['SCRIPT_NAME'], '/wp-admin/plugins.php') !== false) {
    ...
    public static function pre_check_pingback($method)
    {
        ...
        if ($wp_xmlrpc_server->message->methodName === 'system.multicall') {
            $is_multicall = true;
            ...
            foreach ($wp_xmlrpc_server->message->params as $xmlrpc_action) {
                if ($xmlrpc_action['methodName'] === 'pingback.ping') {
        ...
        if (!empty($pingback_args[1])) {
            ...
            if (isset($comment['akismet_result']) && $comment['akismet_result'] == 'true') {
    ...
    {
        if (apply_filters('akismet_comment_form_privacy_notice', get_option('akismet_comment_form_privacy_notice', 'hide')) !== 'display') {
            return;
        }
        echo apply_filters(
    ...
    public static function load_form_js()
    {
        if (
            ...
            wp_register_script('akismet-frontend', plugin_dir_url(__FILE__) . '_inc/akismet-frontend.js', [], filemtime(plugin_dir_path(__FILE__) . '_inc/akismet-frontend.js'), true);
    ...
    {
        if (in_array($tag, ['contact-form', 'gravityform', 'contact-form-7', 'formidable', 'fluentform'])) {
            self::load_form_js();
        }
