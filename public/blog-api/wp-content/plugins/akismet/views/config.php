				...
				<div class="inside">
					<form action="<?php echo esc_url(Akismet_Admin::get_page_url()); ?>" autocomplete="off" method="POST" id="akismet-settings-form">
						...
						<div class="akismet-settings">
			                ...
			                checked(true, (in_array(get_option('akismet_show_user_comments_approved'), [false, '1', 'true'], true)));
		    ?>
										/>
									...
									<fieldset>
										...
										<div>
											<label class="akismet-settings__row-input-label" for="akismet_comment_form_privacy_notice_hide">
												<input type="radio" name="akismet_comment_form_privacy_notice" id="akismet_comment_form_privacy_notice_hide" value="hide" <?php echo in_array(get_option('akismet_comment_form_privacy_notice'), ['display', 'hide']) ? checked('hide', get_option('akismet_comment_form_privacy_notice'), false) : 'checked="checked"'; ?> /> 
												<span class="akismet-settings__row-label-text">
						...
						<table class="akismet-account">
							<tbody>
								...
								<tr>
									...
									<td>
										<?php
		    if ($akismet_user->status == 'cancelled') {
		        esc_html_e('Cancelled', 'akismet');
		    } elseif ($akismet_user->status == 'suspended') {
		        esc_html_e('Suspended', 'akismet');
		    } elseif ($akismet_user->status == 'missing') {
		        esc_html_e('Missing', 'akismet');
		    } elseif ($akismet_user->status == 'no-sub') {
			    ...
			    ?>
						...
						<div class="akismet-card-actions">
							<div id="publishing-action">
								<?php Akismet::view('get', ['text' => ($akismet_user->account_type == 'free-api-key' && $akismet_user->status == 'active' ? __('Upgrade', 'akismet') : __('Change', 'akismet')), 'redirect' => 'upgrade']); ?>
							</div>
							<div class="clear"></div>
						</div>
					</div>
