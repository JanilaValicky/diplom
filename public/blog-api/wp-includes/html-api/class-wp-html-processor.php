...
class WP_HTML_Processor extends WP_HTML_Tag_Processor
{
    ...
    public function next_tag($query = null)
    {
        ...
        if (isset($query['tag_closers']) && $query['tag_closers'] === 'visit') {
            _doing_it_wrong(
                __METHOD__,
        ...
        while ($match_offset > 0 && $this->step()) {
            if ($this->matches_breadcrumbs($breadcrumbs) && --$match_offset === 0) {
                return true;
            }
        }
    ...
    public function step($node_to_process = self::PROCESS_NEXT_NODE)
    {
        ...
        if ($node_to_process === self::PROCESS_NEXT_NODE) {
            /*
    ...
    private function reconstruct_active_formatting_elements()
    {
         ...
         */
        if ($this->state->active_formatting_elements->count() === 0) {
            return false;
        }
