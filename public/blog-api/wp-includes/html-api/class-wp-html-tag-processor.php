...
class WP_HTML_Tag_Processor
{
    ...
    public function next_tag($query = null)
    {
        ...
        do {
            ...
            if (
                ...
                if ($tag_name === 'SCRIPT' && !$this->skip_script_data()) {
                    $this->bytes_already_parsed = strlen($this->html);

                    return false;
                } elseif (
                    ($tag_name === 'TEXTAREA' || $tag_name === 'TITLE') &&
    ...
    private function parse_next_attribute()
    {
        ...
        if ($name_length === 0 || $this->bytes_already_parsed + $name_length >= strlen($this->html)) {
            return false;
        }
    ...
    {
        if ($this->is_closing_tag || $this->tag_name_starts_at === null) {
            return null;
        }
    ...
    public function remove_attribute($name)
    {
        ...
        if ($this->duplicate_attributes !== null && array_key_exists($name, $this->duplicate_attributes)) {
    ...
    private function parse_query($query)
    {
        ...
        if (isset($query['match_offset']) && is_int($query['match_offset']) && $query['match_offset'] > 0) {
