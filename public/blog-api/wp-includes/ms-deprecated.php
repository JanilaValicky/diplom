...
<p class="message">%s</p>
        ...
        exit(sprintf($message_template, $message));
    }
}
...
function install_blog($blog_id, $blog_title = '')
{
    ...
    if (!is_subdomain_install()) {
        ...
        if (parse_url(get_site_option('siteurl'), PHP_URL_SCHEME) === 'https') {
            $siteurl = set_url_scheme($siteurl, 'https');
        }
        if (parse_url(get_home_url(get_network()->site_id), PHP_URL_SCHEME) === 'https') {
