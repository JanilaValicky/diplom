...
function get_oembed_response_data_rich($data, $post, $width, $height)
{
    ...
    if ($thumbnail_id) {
        [$thumbnail_url, $thumbnail_width, $thumbnail_height] = wp_get_attachment_image_src($thumbnail_id, [$width, 99999]);
...
function _oembed_rest_pre_serve_request($served, $result, $request, $server)
{
    ...
    if (!class_exists('SimpleXMLElement')) {
        status_header(501);
        exit(get_status_header_desc(501));
    }
...
{
    if ($result === false || !in_array($data->type, ['rich', 'video'], true)) {
        return $result;
    }
...
function wp_filter_oembed_result($result, $data, $url)
{
    ...
    if ($wp_oembed->get_provider($url, ['discover' => false]) !== false) {
        return $result;
    }
