...
class wp_xmlrpc_server extends IXR_Server
{
    ...
    {
        if ($name === '_multisite_getUsersBlogs') {
    ...
    protected function _insert_post($user, $content_struct)
    {
        ...
        if ($post_data['comment_status'] !== 'open' && $post_data['comment_status'] !== 'closed') {
            unset($post_data['comment_status']);
        }

        if ($post_data['ping_status'] !== 'open' && $post_data['ping_status'] !== 'closed') {
    ...
    public function wp_getTaxonomies($args)
    {
        ...
        if (isset($args[4])) {
            ...
            /** This action is documented in wp-includes/class-wp-xmlrpc-server.php */
            $fields = apply_filters('xmlrpc_default_taxonomy_fields', ['labels', 'cap', 'object_type'], 'wp.getTaxonomies');
        }
    ...
    public function wp_getPostTypes($args)
    {
        ...
        if (isset($args[4])) {
            ...
            /** This action is documented in wp-includes/class-wp-xmlrpc-server.php */
            $fields = apply_filters('xmlrpc_default_posttype_fields', ['labels', 'cap', 'taxonomies'], 'wp.getPostTypes');
        }
    ...
    public function blogger_editPost($args)
    {
        ...
        if ($actual_post['post_status'] === 'publish' && !current_user_can('publish_posts')) {
    ...
    public function mw_newPost($args)
    {
        ...
        if (!empty($content_struct['post_type'])) {
            if ($content_struct['post_type'] === 'page') {
                if ($publish) {
                    $cap = 'publish_pages';
                } elseif (isset($content_struct['page_status']) && $content_struct['page_status'] === 'publish') {
                    $cap = 'publish_pages';
                } else {
                    $cap = 'edit_pages';
                }
                ...
                if ($publish) {
                    $cap = 'publish_posts';
                } elseif (isset($content_struct['post_status']) && $content_struct['post_status'] === 'publish') {
                    $cap = 'publish_posts';
                } else {
                    $cap = 'edit_posts';
                }
    ...
    public function mw_editPost($args)
    {
        ...
        $page_template = '';
        if (!empty($content_struct['wp_page_template']) && $post_type === 'page') {
            $page_template = $content_struct['wp_page_template'];
        }
        ...
        if ($post_status === 'publish' || $post_status === 'private') {
            if ($post_type === 'page' && !current_user_can('publish_pages')) {
    ...
    public function mw_getPost($args)
    {
        ...
        if ($postdata['post_date'] !== '') {
            ...
            $allow_comments = ($postdata['comment_status'] === 'open') ? 1 : 0;
            $allow_pings = ($postdata['ping_status'] === 'open') ? 1 : 0;
    ...
    public function pingback_ping($args)
    {
        ...
        $pos1 = strpos($pagelinkedto, str_replace(['http://www.', 'http://', 'https://www.', 'https://'], '', get_option('home')));
        if (!$pos1) {
