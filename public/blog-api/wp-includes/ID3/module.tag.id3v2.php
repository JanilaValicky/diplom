...
class getid3_id3v2 extends getid3_handler
{
    ...
    public function Analyze()
    {
        ...
        if (isset($thisfile_id3v2['comments']['track_number'])) {
            foreach ($thisfile_id3v2['comments']['track_number'] as $key => $value) {
                if (strstr($value, '/')) {
                    [$thisfile_id3v2['comments']['track_number'][$key], $thisfile_id3v2['comments']['totaltracks'][$key]] = explode('/', $thisfile_id3v2['comments']['track_number'][$key]);
                }
            }
        }
    ...
    public function ParseID3v2Frame(&$parsedFrame)
    {
        ...
        //unset($parsedFrame['data']); do not unset, may be needed elsewhere, e.g. for replaygain
                ...
                while ($frame_offset < strlen($parsedFrame['data'])) {
                    ...
                    switch ($subframe['name']) {
                        ...
                        case 'TIT3':
                            ...
                            break;
                        case 'WXXX':
                            @[$subframe['chapter_url_description'], $subframe['chapter_url']] = explode("\x00", $encoding_converted_text, 2);
            ...
            $id3v2_chapter_entry = [];
            foreach (['id', 'time_begin', 'time_end', 'offset_begin', 'offset_end', 'chapter_name', 'chapter_description', 'chapter_url', 'picture_present'] as $id3v2_chapter_key) {
