...
function wp_admin_bar_dashboard_view_site_menu($wp_admin_bar)
{
    ...
    if ($user_id != 0) {
        if (is_admin()) {
            ...
            $wp_admin_bar->add_menu(['id' => 'dashboard', 'title' => __('Dashboard'), 'href' => get_dashboard_url($user_id)]);
        } else {
...
function wp_get_http($url, $file_path = false, $red = 1)
{
    ...
    if ($options['method'] == 'HEAD' && in_array($headers['response'], [301, 302]) && isset($headers['location'])) {
...
function wp_get_loading_attr_default($context)
{
    ...
    if ($context === 'the_post_thumbnail' || $context === 'wp_get_attachment_image') {
        /*
     ...
     */
    if ($context === 'the_content' || $context === 'the_post_thumbnail') {
...
{
    _deprecated_function(__FUNCTION__, '6.3.0');
    if (gettype($n) === 'string' && str_contains($n, '.') && (float) $n === 1) {
        $n = '100%';
    }
    ...
    if (gettype($n) === 'string' && str_contains($n, '%')) {
        $n = (int) ($n * $max) / 100;
    }
