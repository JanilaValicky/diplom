...
function wp_preload_resources()
{
    ...
    foreach ($unique_resources as $unique_resource) {
        ...
        foreach ($unique_resource as $resource_key => $resource_value) {
            ...
            if (($resource_key === 'imagesrcset') && (!isset($unique_resource['as']) || ($unique_resource['as'] !== 'image'))) {
                continue;
            }

            // imagesizes only usable when preloading image and imagesrcset present, ignore otherwise.
            if (($resource_key === 'imagesizes') &&
                (!isset($unique_resource['as']) || ($unique_resource['as'] !== 'image') || !isset($unique_resource['imagesrcset']))
            ) {
                continue;
            }

            $resource_value = ($resource_key === 'href') ? esc_url($resource_value, ['http', 'https']) : esc_attr($resource_value);
