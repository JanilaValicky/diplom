...
function get_comment_author_link($comment_id = 0)
{
    ...
    if (empty($comment_author_url) || $comment_author_url === 'http://') {
...
function get_comment_author_url($comment_id = 0)
{
    ...
    if (!empty($comment)) {
        $comment_author_url = ($comment->comment_author_url === 'http://') ? '' : $comment->comment_author_url;
...
function get_comments_number_text($zero = false, $one = false, $more = false, $post = 0)
{
    ...
    if ($comments_number > 1) {
        ...
        $comments_number_text = ($zero === false) ? __('No Comments') : $zero;
    } else { // Must be one.
        $comments_number_text = ($one === false) ? __('1 Comment') : $one;
    }
...
function comments_open($post = null)
{
    ...
    $comments_open = ($_post && ($_post->comment_status === 'open'));
...
function pings_open($post = null)
{
    ...
    $post_id = $_post ? $_post->ID : 0;
    $pings_open = ($_post && ($_post->ping_status === 'open'));
...
function comments_popup_link($zero = false, $one = false, $more = false, $css_class = '', $none = false)
{
    ...
    if ($comments_number == 0 && !comments_open() && !pings_open()) {
        printf(
            '<span%1$s>%2$s</span>',
...
function get_cancel_comment_reply_link($link_text = '', $post = null)
{
    ...
    $link_style = $reply_to_id !== 0 ? '' : ' style="display:none;"';
