...
function wp_get_layout_style($selector, $layout, $has_block_gap_support = false, $gap_value = null, $should_skip_gap_serialization = false, $fallback_gap_value = '0.5em', $block_spacing = null)
{
    ...
    if ($layout_type === 'default') {
        if ($has_block_gap_support) {
            ...
            if ($gap_value !== null && !$should_skip_gap_serialization) {
    ...
    } elseif ($layout_type === 'constrained') {
...
function wp_render_layout_support_flag($block_content, $block)
{
    ...
    if ($layout_from_parent === 'fixed' || $layout_from_parent === 'fill') {
        $container_content_class = wp_unique_id('wp-container-content-');

        $child_layout_styles = [];

        if ($layout_from_parent === 'fixed' && isset($block['attrs']['style']['layout']['flexSize'])) {
            $child_layout_styles[] = [
        ...
        } elseif ($layout_from_parent === 'fill') {
            $child_layout_styles[] = [
                'selector' => ".$container_content_class",
                'declarations' => [
                    'flex-grow' => '1',
    ...
    do {
        ...
        if (strpos($processor->get_attribute('class'), $inner_block_wrapper_classes) !== false) {
            break;
        }
...
function wp_restore_group_inner_container($block_content, $block)
{
    ...
    if (
        ...
        (isset($block['attrs']['layout']['type']) && $block['attrs']['layout']['type'] === 'flex')
    ) {
        return $block_content;
    }
