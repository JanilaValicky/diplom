...
function wp_get_typography_value_and_unit($raw_value, $options = [])
{
     ...
     */
    if ($options['coerce_to'] === 'px' && ($unit === 'em' || $unit === 'rem')) {
        $value = $value * $options['root_size_value'];
        $unit = $options['coerce_to'];
    }

    if ($unit === 'px' && ($options['coerce_to'] === 'em' || $options['coerce_to'] === 'rem')) {
        $value = $value / $options['root_size_value'];
        $unit = $options['coerce_to'];
    }
     ...
     */
    if (($options['coerce_to'] === 'em' || $options['coerce_to'] === 'rem') && ($unit === 'em' || $unit === 'rem')) {
        $unit = $options['coerce_to'];
    }
...
function wp_get_typography_font_size_value($preset, $should_use_fluid_typography = false)
{
    ...
    if (
        ...
        ($typography_settings['fluid'] === true || is_array($typography_settings['fluid']))
    ) {
    ...
    if ($fluid_font_size_settings === false) {
        return $preset['size'];
    }
