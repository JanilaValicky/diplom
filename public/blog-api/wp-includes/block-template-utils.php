...
function _build_block_template_result_from_file($template_file, $template_type)
{
    ...
    if ($template_type === 'wp_template' && isset($template_file['postTypes'])) {
        $template->post_types = $template_file['postTypes'];
    }

    if ($template_type === 'wp_template_part' && isset($template_file['area'])) {
...
function _build_block_template_result_from_post($post)
{
    ...
    if ($parent_post->post_type === 'wp_template' && $has_theme_file && isset($template_file['postTypes'])) {
        $template->post_types = $template_file['postTypes'];
    }

    if ($parent_post->post_type === 'wp_template' && isset($default_template_types[$template->slug])) {
        $template->is_custom = false;
    }

    if ($parent_post->post_type === 'wp_template_part') {
    ...
    if ($parent_post->post_type === 'wp_template' && empty($template->description) && (empty($template->title) || $template->title === $template->slug)) {
        $matches = [];
            ...
            switch ($type) {
                ...
                case 'single':
                    ...
                    foreach ($post_types as $post_type) {
                        ...
                        if (strncmp($slug_remaining, $post_type . '-', $post_type_length) === 0) {
                ...
                case 'taxonomy':
                    ...
                    foreach ($taxonomies as $taxonomy) {
                        ...
                        if (strncmp($slug_remaining, $taxonomy . '-', $taxonomy_length) === 0) {
...
function get_block_templates($query = [], $template_type = 'wp_template')
{
    ...
    if ($template_type === 'wp_template_part' && isset($query['area'])) {
...
function get_template_hierarchy($slug, $is_custom = false, $template_prefix = '')
{
    ...
    if (!empty($template_prefix)) {
        ...
        $items = $type === 'single' ? get_post_types() : get_taxonomies();
