...
function is_active_widget($callback = false, $widget_id = false, $id_base = false, $skip_inactive = true)
{
    ...
    if (is_array($sidebars_widgets)) {
        foreach ($sidebars_widgets as $sidebar => $widgets) {
            if ($skip_inactive && ($sidebar === 'wp_inactive_widgets' || str_starts_with($sidebar, 'orphaned_widgets'))) {
                continue;
            }
...
function wp_map_sidebars_widgets($existing_sidebars_widgets)
{
    ...
    foreach ($existing_sidebars_widgets as $sidebar => $widgets) {
        if ($sidebar === 'wp_inactive_widgets' || str_starts_with($sidebar, 'orphaned_widgets')) {
    ...
    if (count($existing_sidebars_widgets) === 1 && count($wp_registered_sidebars) === 1) {
        ...
        foreach ($common_slug_groups as $slug_group) {
            ...
            foreach ($slug_group as $slug) {
                ...
                foreach ($wp_registered_sidebars as $new_sidebar => $args) {
                    ...
                    // ...actually match!
                    if (stripos($new_sidebar, $slug) === false && stripos($slug, $new_sidebar) === false) {
                        continue;
                    }
                    ...
                    foreach ($existing_sidebars_widgets as $sidebar => $widgets) {
                        ...
                        foreach ($slug_group as $slug) {
                            ...
                            if (stripos($sidebar, $slug) === false && stripos($slug, $sidebar) === false) {
                                continue;
                            }
                ...
                foreach ($new_sidebars_widgets as $new_sidebar => $new_widgets) {
                    ...
                    foreach ($old_widgets as $key => $widget_id) {
                        ...
                        if ($active_key !== false) {
                            ...
                            if ($new_sidebar === 'wp_inactive_widgets') {
...
<?php } if ($inputs['show_summary'] || $inputs['show_author'] || $inputs['show_date']) { ?>
	<p>
...
function wp_render_widget($widget_id, $sidebar_id)
{
    ...
    } elseif ($sidebar_id === 'wp_inactive_widgets') {
        $sidebar = [];
    } else {
        return '';
    }
