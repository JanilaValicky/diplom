...
function wp_list_users($args = [])
{
    ...
    foreach ($users as $user_id) {
        ...
        if ($parsed_args['exclude_admin'] && $user->display_name === 'admin') {
            continue;
        }
        ...
        $return .= $row;
        $return .= ($parsed_args['style'] === 'list') ? '</li>' : ', ';
    }
...
function _wp_privacy_send_request_confirmation_notification($request_id)
{
    ...
    if (!($request instanceof WP_User_Request) || $request->status !== 'request-confirmed') {
        return;
    }
...
function _wp_privacy_send_erasure_fulfillment_notification($request_id)
{
    ...
    if (!($request instanceof WP_User_Request) || $request->status !== 'request-completed') {
        return;
    }
