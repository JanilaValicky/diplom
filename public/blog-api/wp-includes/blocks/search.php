...
function render_block_core_search($attributes, $content, $block)
{
    ...
    $show_button = (!empty($attributes['buttonPosition']) && $attributes['buttonPosition'] === 'no-button') ? false : true;
    ...
    $is_button_inside = !empty($attributes['buttonPosition']) &&
        $attributes['buttonPosition'] === 'button-inside';
    ...
    if ($input->next_tag()) {
        ...
        $is_expandable_searchfield = $button_position === 'button-only' && $button_behavior === 'expand-searchfield';
    ...
    if ($show_button) {
        ...
        if ($button->next_tag()) {
            ...
            if ($attributes['buttonBehavior'] === 'expand-searchfield' && $attributes['buttonPosition'] === 'button-only') {
...
function classnames_for_block_core_search($attributes)
{
    ...
    if (!empty($attributes['buttonPosition'])) {
        ...
        if ($attributes['buttonPosition'] === 'button-outside') {
            $classnames[] = 'wp-block-search__button-outside';
        }

        if ($attributes['buttonPosition'] === 'no-button') {
            $classnames[] = 'wp-block-search__no-button';
        }

        if ($attributes['buttonPosition'] === 'button-only') {
            $classnames[] = 'wp-block-search__button-only';
            if (!empty($attributes['buttonBehavior']) && $attributes['buttonBehavior'] === 'expand-searchfield') {
    ...
    if (isset($attributes['buttonUseIcon'])) {
        if (!empty($attributes['buttonPosition']) && $attributes['buttonPosition'] !== 'no-button') {
...
{
    $is_button_inside = isset($attributes['buttonPosition']) && $attributes['buttonPosition'] === 'button-inside';
