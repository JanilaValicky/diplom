...
function block_core_navigation_add_directives_to_submenu($w, $block_attributes)
{
    while ($w->next_tag(
        ...
        if (!isset($block_attributes['openSubmenusOnClick']) || $block_attributes['openSubmenusOnClick'] === false) {
...
function block_core_navigation_from_block_get_post_ids($block)
{
    ...
    if ($block->name === 'core/navigation-link' || $block->name === 'core/navigation-submenu') {
        if ($block->attributes && isset($block->attributes['kind']) && $block->attributes['kind'] === 'post-type' && isset($block->attributes['id'])) {
...
function render_block_core_navigation($attributes, $content, $block)
{
    ...
    if (array_key_exists('ref', $attributes)) {
        ...
        if ($navigation_post->post_status === 'publish') {
...
function block_core_navigation_get_classic_menu_fallback()
{
    ...
    if ($classic_nav_menus && !is_wp_error($classic_nav_menus)) {
        ...
        foreach ($classic_nav_menus as $classic_nav_menu) {
            if ($classic_nav_menu->slug === 'primary') {
