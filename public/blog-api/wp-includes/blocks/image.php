...
function render_block_core_image($attributes, $content, $block)
{
    ...
    if (!$processor->next_tag('img') || $processor->get_attribute('src') === null) {
        return '';
    }
    ...
    if (
        ...
        $lightbox_settings['enabled'] === true
    ) {
        $block->block_type->supports['interactivity'] = true;

        if (!in_array($view_js_file_handle, $script_handles, true)) {
            $block->block_type->view_script_handles = array_merge($script_handles, [$view_js_file_handle]);
        }
        ...
        if (in_array($view_js_file_handle, $script_handles, true)) {
            $block->block_type->view_script_handles = array_diff($script_handles, [$view_js_file_handle]);
        }
    }
...
function block_core_image_render_lightbox($block_content, $block)
{
     ...
     */
    if (stripos($block_content, '<img') === false) {
